...
// Resolve internal package paths blocked by exports maps — follow the symlink to the real path
function resolveDeepPath(packageName: string, subpath: string): string {
	const link = join(process.cwd(), "node_modules", packageName);
	const real = realpathSync(link);
	return join(real, subpath);
}
...
test("bash autocomplete providers return null synchronously in shell contexts", () => {
	...
	const bashSuggestions = new BashAutocompleteProvider().getSuggestions(
		["git st"],
		0,
		6,
		{ signal },
	);
	const oneOffSuggestions = new OneOffBashAutocompleteProvider().getSuggestions(
		["!git st"],
		0,
		7,
		{ signal },
...
test("bash editor runs copied Pi app action handlers for alt-enter", async () => {
	...
	const { setKittyProtocolActive } = await import(
		resolveDeepPath("@earendil-works/pi-tui", "dist/keys.js")
	);
