...
// Regex-safe marker for per-message usage footer. Wrapped in HTML-comment
// delimiters so it can be reliably stripped from LLM context without false
// positives. The `context` handler removes any match before the LLM sees it.
const USAGE_FOOTER_RE = /<!-- pi-usage:[\s\S]*?-->/g;
function wrapUsageFooter(text: string): string {
	return `\n<!-- pi-usage: ${text} -->`;
}

function formatUsageFooter(
	usage: SessionAssistantUsage,
	tokenRate?: number | null,
): string {
	const icons = getIcons();
	const parts: string[] = [];
	if (usage.input > 0) parts.push(`${icons.input}${formatTokens(usage.input)}`);
	if (usage.output > 0)
		parts.push(`${icons.output}${formatTokens(usage.output)}`);
	if (usage.cacheRead > 0)
		parts.push(`${icons.cache} ${icons.input}${formatTokens(usage.cacheRead)}`);
	if (usage.cacheWrite > 0)
		parts.push(
			`${icons.cache} ${icons.output}${formatTokens(usage.cacheWrite)}`,
		);
	if (tokenRate != null && tokenRate > 0)
		parts.push(`${icons.tokenRate}${formatTokens(Math.round(tokenRate))}/s`);
	if (usage.cost.total > 0) {
		parts.push(
			`${icons.cost}${usage.cost.total < 0.01 ? usage.cost.total.toFixed(4) : usage.cost.total.toFixed(2)}`,
		);
	}
	return parts.join("  ");
}
...
function segmentSeparatorWidth(
...
function buildJustifiedTopContent(
	...
	const left = buildGroupContentFromParts(
		leftSegments.map((segment) => segment.content),
		presetDef,
	);
	const right = buildGroupContentFromParts(
		rightSegments.map((segment) => segment.content),
		presetDef,
	);
	...
	if (left && right) {
		const gapWidth = Math.max(
			JUSTIFY_MIN_GAP,
			availableWidth - leftWidth - rightWidth - 2,
		);
...
function renderSegmentsWithWidth(
	segIds: readonly StatusLineSegmentId[],
	ctx: SegmentContext,
): LayoutSegment[] {
...
function computeResponsiveLayout(
	...
	const topLeftSegments = renderSegmentsWithWidth(
		mergedSegments.leftSegments,
		ctx,
	);
	const topRightSegments = renderSegmentsWithWidth(
		mergedSegments.rightSegments,
		ctx,
	);
	const secondaryBaseSegments = renderSegmentsWithWidth(
	...
	while (
		topContentWidth(topLeftSegments, topRightSegments, presetDef) >
			availableWidth &&
	...
	const secondarySegments = fitSegmentsIntoRow(
		[
			...overflowLeftSegments,
			...overflowRightSegments,
			...secondaryBaseSegments,
		],
		presetDef,
		availableWidth,
	);
	...
	return {
		topContent: buildJustifiedTopContent(
			topLeftSegments,
			topRightSegments,
			presetDef,
			availableWidth,
		),
		secondaryContent: buildContentFromParts(
			secondarySegments.map((segment) => segment.content),
			presetDef,
		),
	};
}
...
export default function powerlineFooter(pi: ExtensionAPI) {
	...
	pi.on("message_end", async (event, ctx) => {
		currentCtx = ctx;
		let modifiedMessage: AssistantMessage | undefined;
		if (isSessionAssistantMessage(event.message)) {
			if (
				...
				if (
					...
					tokenRate = (event.message.usage.output * 1000) / duration;
					persistedTokenRate = tokenRate;
				}

				// Append per-message usage footer for display (stripped from LLM context by `context` handler)
				const footer = formatUsageFooter(event.message.usage, tokenRate);
				if (footer) {
					const content = [...event.message.content];
					// Find the last text block to append to, or create one
					let lastTextIndex = -1;
					...
					if (lastTextIndex >= 0) {
						const originalBlock = content[lastTextIndex] as {
							type: "text";
							text: string;
						};
						content[lastTextIndex] = {
							...originalBlock,
							text: originalBlock.text + wrapUsageFooter(footer),
						};
					} else {
						content.push({
							type: "text" as const,
							text: wrapUsageFooter(footer),
						});
					}
					modifiedMessage = { ...event.message, content };
				}
			}
		}
		requestImmediateStatusRender({ deferDuringTyping: false });
	...
	// Strip per-message usage footers from LLM context.
	// Usage footers are appended by `message_end` for display but must never be
	// sent to the LLM — they waste tokens. The regex matches the exact marker
	// format, which will never appear in normal LLM output.
		...
		for (const msg of messages) {
			...
			for (const block of content) {
				if (block.type === "text" && typeof block.text === "string") {
					block.text = block.text.replace(USAGE_FOOTER_RE, "").trimEnd();
				}
			}
		}
		return { messages };
